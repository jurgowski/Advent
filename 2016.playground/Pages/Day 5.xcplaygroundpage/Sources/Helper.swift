...
public func password(_ doorId: String, need: Int) -> String
{
    var pass = ""
    var letters = 0
    var salt = 0
    while(letters < need) {
        let hash = "\(doorId)\(salt)".md5
        if hash.hasPrefix("00000") {
            pass.append(hash[hash.index(hash.startIndex, offsetBy: 5)])
            letters += 1
            print(pass)
        }
        salt += 1
...
public func strongerPassword(_ doorId: String, need: Int) -> String
{
    var passLetters: [Int:Character] = [:]
    var salt = 0
    while(passLetters.count < need) {
        salt += 1
        if (salt % 1000000 == 0) {
            print(salt)
        }
        ...
        if hash.hasPrefix("00000") {
            ...
            let position = Int(String(hash[positionIndex]))
            if (position == nil || position! >= need || passLetters[position!] != nil) {
                print("skip \(hash[positionIndex])")
                continue
            }
            let letterIndex = hash.index(positionIndex, offsetBy: 1)
            passLetters[position!] = hash[letterIndex]
            print(passLetters)
        }
    }
    return constructPass(passLetters)
}

public func constructPass(_ map: [Int : Character]) -> String {
    var pass = ""
