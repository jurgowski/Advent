...
private func solve(input: String, prizeBoost: Int = 0) -> Int {
    let machines = input
        .split(separator: "\n\n")
    
    return machines.reduce(0) { partial, machineInput in
        let machine = machineInput.split(separator: "\n")
        let buttonA = machine[0].split(separator: ": ")[1].split(separator:", ").map { Double($0.split(separator: "+")[1])! }
        let buttonB = machine[1].split(separator: ": ")[1].split(separator:", ").map { Double($0.split(separator: "+")[1])! }
        let prize = machine[2].split(separator: ": ")[1].split(separator:", ").map { Double($0.split(separator: "=")[1])! + Double(prizeBoost) }
        ...
        guard let xInt = Int(exactly: x) , let yInt = Int(exactly: y) else { return partial }
        return partial + xInt * 3 + yInt
    }
}
...
@Test func testDay13DebugPart1() async throws {
    #expect(solve(input: debugInput) == 480)
}

@Test func testDay13Part1() throws {
    print(solve(input: input))
}

@Test func testDay13Part2() async throws {
    #expect(solve(input: debugInput, prizeBoost: 10000000000000) == 875318608908)
}

@Test func testDay13DebugPart2() async throws {
    print(solve(input: input, prizeBoost: 10000000000000))
}
